...
Rational Rational::mySqrt()
{
	...
	int denom = pVal / gcdVal;
	int numer = round(frac * pVal) / gcdVal + integral * denom;
 ...
 void Rational::quadraticEquations(const Rational& a, const Rational& b, const Rational& c)
{
	 Rational discriminant = b * b - a * c * 4;

	 if (discriminant < 0)
		 cout << "Discriminant < 0" << endl;
	 else {
		 Rational x1 = (-b - discriminant.mySqrt()) / (a * 2);
		 Rational x2 = (-b + discriminant.mySqrt()) / (a * 2);
